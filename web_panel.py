...
def _safe_config(config):
    """Return config with API key replaced by boolean flag. Mutates the passed dict."""
    key = config.pop("sistrix_api_key", "")
    config["has_api_key"] = bool(key and key != "TU_API_KEY_AQUI")
    return config


@app.route("/api/config", methods=["GET"])
def get_config():
    return jsonify(_safe_config(load_config()))
...
def update_display():
    ...
    if "screen_off" in data:
        config["display"]["screen_off"] = bool(data["screen_off"])
    save_config(config)
...
def get_preview_data_internal(config, force=False, refresh=False):
    ...
    results = []
    for idx, d, data in _executor.map(fetch_one, active):
        if data:
            change = 0
 ...
 * { margin:0; padding:0; box-sizing:border-box; }
 ...
 .touch-hit::before { content:''; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); min-width:44px; min-height:44px; }
 body {
 ...
 .led-info { font-size:var(--text-sm); color:var(--dim); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; min-width:0; }
 /* Domain cards */
 .domain-card {
 ...
 .btn-icon-danger.armed { background:var(--red); color:white; opacity:1; border-color:var(--red); font-size:var(--text-xs); width:auto; padding:0 var(--space-3); letter-spacing:0.5px; }
 ...
 .main-layout { display:grid; grid-template-columns:minmax(0,1fr); gap:var(--space-6); }
 @media (min-width:900px) {
 .main-layout { grid-template-columns:minmax(0,1fr) minmax(0,1fr); align-items:start; }
 ...
 .led-stage { max-width:100%; }
 .led-outer { padding:var(--space-2); max-width:100%; box-sizing:border-box; }
...
<main class="main-layout">
 ...
 <div class="led-status-row"><span id="previewStatus" class="led-info" aria-live="polite"></span><button id="btnPower" class="btn-power" onclick="toggleScreen()" aria-label="Toggle screen">⏻</button></div>
...
const I18N = {
 ...
 confirm_delete:'¿Eliminar?', enable:'Activar', disable:'Desactivar', fill_fields:'Rellena dominio y label', refresh_confirm_short:'Añadirá solo los datos faltantes · Clica de nuevo para confirmar', credits_available:'créditos disponibles', apikey_removed:'API Key eliminada', apikey_checking:'Validando API Key...', apikey_valid:'API Key válida', apikey_invalid:'API Key no válida', credits:'créditos', credits_used:'créditos consumidos', loading_data_short:'Cargando datos...',
 ...
 confirm_delete:'Delete?', enable:'Enable', disable:'Disable', fill_fields:'Fill in domain and label', refresh_confirm_short:'Will only add missing data · Click again to confirm', credits_available:'credits left', apikey_removed:'API Key removed', apikey_checking:'Validating API Key...', apikey_valid:'API Key valid', apikey_invalid:'Invalid API Key', credits:'credits', credits_used:'credits used', loading_data_short:'Loading data...',
...
function initFormSelects() {
 initCustomSelect(DOM.newMode, [{value:'weekly',text:t('mode_weekly')},{value:'daily',text:t('mode_daily')}], DOM.newMode.value || 'weekly');
 initCustomSelect(DOM.newType, [{value:'domain',text:t('domain')},{value:'host',text:'Host'},{value:'path',text:'Path'},{value:'url',text:'URL'}], DOM.newType.value || 'domain');
}
...
function fmtChange(pct) { return (pct >= 0 ? '+' : '') + pct.toFixed(1) + '%'; }
...
function drawLED(data) {
 ...
 // Value
 const vh = DL.valueH;
 const valueStr = fmtValue(data.current_value);
...
function renderSlide() {
 ...
 if (d) {
 canvas.setAttribute('aria-label', `${d.label}: ${d.current_value ?? ''} - ${d.domain} ${d.country.toUpperCase()} ${d.mode === 'daily' ? 'D' : 'W'}`);
 }
 }
}
...
async function doRefresh() {
 ...
 try {
 const creditsBefore = sistrixCredits;
 const snapshot = previewData.map(d => `${d.label}:${d.current_value}`).join('|');
 await updatePreviewData(false, true);
 await loadCacheStatus();
 const creditsRes = await fetch('/api/credits').then(r => r.json());
 sistrixCredits = creditsRes.credits; updateStatusBar();
 const newSnapshot = previewData.map(d => `${d.label}:${d.current_value}`).join('|');
 if (snapshot === newSnapshot) {
  toast(t('no_changes'));
 } else {
  let msg = t('data_updated');
  if (creditsBefore != null && sistrixCredits != null) {
   const used = creditsBefore - sistrixCredits;
   msg += ` · ${used.toLocaleString('de-DE')} ${t('credits_used')} · ${sistrixCredits.toLocaleString('de-DE')} ${t('credits_available')}`;
  }
  toast(msg);
 }
 } catch(e) {
...
function applyConfig(config) {
 ...
 // Restore screen off state
 const wasOff = !!config.display.screen_off;
 const btn = $('btnPower');
 if (wasOff && !screenOff) {
  screenOff = true;
  stopRotation();
...
async function reorderDomain(fromIdx, toIdx) {
...
function initDragAndDrop() {
 ...
 cards.forEach(card => {
  ...
  card.addEventListener('drop', async e => {
   ...
   await reorderDomain(_dragSrcIndex, to);
  });
...
document.addEventListener('touchend', async () => {
 ...
 if (overCard) {
  ...
  if (_touchIndex !== null && _touchIndex !== to) {
   await reorderDomain(_touchIndex, to);
  }
 }
...
// ===== BRAND =====
const DEFAULT_BRAND_LAYOUT = { logoX:1, logoY:1, logoSize:16, nameX:20, nameY:6, nameH:7, nameScale:1, msgX:0, msgY:21, msgH:8, msgScale:1, nameColor:'#ffffff', msgColor:'rainbow', msgSpeed:42, nameFont:'small', msgFont:'small' };
let brandData = {};
let brandLogoPixels = null;
let brandLayout = { ...DEFAULT_BRAND_LAYOUT };
...
function showEditPopup(screenX, screenY, currentVal, onConfirm, opts) {
 ...
 trapFocus(popup, 'input, button, [tabindex]:not([tabindex="-1"]), .color-swatch, .color-custom-dot');
...
function showLogoPopup(screenX, screenY) {
 ...
 popup.addEventListener('mousedown', e => e.stopPropagation());
 popup.addEventListener('touchstart', e => e.stopPropagation());
 trapFocus(popup, 'input:not([type="file"]), button, label.btn-outline');
 setTimeout(() => { document.addEventListener('mousedown', _onDocClickClose); document.addEventListener('touchstart', _onDocClickClose); }, 0);
}
...
async function resetCurrentLayout() {
 if (layoutEditMode) {
 Object.assign(brandLayout, DEFAULT_BRAND_LAYOUT);
...
function initCustomSelect(container, options, defaultVal) {
 ...
 const alabel = container.getAttribute('aria-label');
 const alabelled = container.getAttribute('aria-labelledby');
 if (alabel) trigger.setAttribute('aria-label', alabel);
 else if (alabelled) trigger.setAttribute('aria-labelledby', alabelled);
